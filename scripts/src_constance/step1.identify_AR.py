...
model = sys.argv[1]
AR_method_tag = sys.argv[2]  # abs or p85
...
def crt_filenames(model, year, month):
    ...
    WRFdir = '/pic/projects/hyperion/chen423/data/papers/AR-SST/data/%s/by_month_NARRgrid/' % model        
    WRFfile = WRFdir + 'WRF_%s.IWV_uvIVT.NARRgrid.6hr.%d.%d.nc' % (model, year, month)
    
    return NARRfile, WRFfile

def gen_mod_NARRdata(model, year, month):
    
    NARRfile, WRFfile = crt_filenames(model, year, month)
...
def get_AR_thre_maps():
    ...
    file_WRF_IWV = '/pic/projects/hyperion/chen423/data/papers/AR-SST/data/HIST/ref/WRF_IWV.NARRgrid.1981-2015.6hr.pctl85.nc'
    file_WRF_IVT = '/pic/projects/hyperion/chen423/data/papers/AR-SST/data/HIST/ref/WRF_uvIVT.NARRgrid.1981-2015.6hr.pctl85.nc'
    ...
    map_IWV_raw[np.where(np.isfinite(fix_IWV)==True)] = fix_IWV[np.where(np.isfinite(fix_IWV)==True)]
    map_IVT_raw[np.where(np.isfinite(fix_IVT)==True)] = fix_IVT[np.where(np.isfinite(fix_IVT)==True)]
...
def idenfity_AR(model, year, month, AR_method_tag=AR_method_tag):
    ...
    # write results
    print('  writing ...')
    outfile = '/pic/projects/hyperion/chen423/data/papers/AR-SST/data/%s/AR_tagged/Gershunov/NARRgrid/WRF_NARRgrid_ARtag.%s.Gershunov.%d.%d.AR%s.nc' % (model, model, year, month, AR_method_tag)
...
for month in np.arange(10,13):
	...
	idenfity_AR(model, 2003, month, AR_method_tag=AR_method_tag)


for year in np.arange(2004, 2016):
	for month in np.arange(1,13):
		print(model, year, month)
		idenfity_AR(model, year, month, AR_method_tag=AR_method_tag)
