...
def gen_raw_NARRdata(model, year, month):
    ...
    data_IVT[np.isinf(data_IVT)] = 0
    data_IWV = get_NARRraw_data(NARRfile, 'IWV')
    data_IWV[np.isinf(data_IWV)] = 0
...
def concatenate_file(model, year, month):
    ...
    if year==2003 and month==10:
        ...
        IVTdata, IWVdata = gen_raw_NARRdata(model, year, month)
        IVTdatan, IWVdatan = gen_raw_NARRdata(model, yearn, monthn)
        ...
        IVTdata, IWVdata = gen_raw_NARRdata(model, year, month)
        IVTdatap, IWVdatap = gen_raw_NARRdata(model, yearp, monthp)
...
def idenfity_AR(model, year, month):
    ...
    # write results
    print('  writing ...')
    outfile = '/pic/projects/hyperion/chen423/data/papers/AR-SST/data/%s/AR_tagged/Gershunov/NARR_raw/NARRraw_ARtag.6hrly.Gershunov.%d.%d.nc' % (model, year, month)
...
for month in np.arange(10,13):
	print(model, 2012, month)
	idenfity_AR(model, 2012, month)
